...
This is still a baseline research system. Important limitations:

### 1. Embeddings are handcrafted, not learned

The current representation is a `librosa` baseline. It is useful for iteration, but not a strong claim about the state of the art in music similarity.

### 2. Upstream stages are heuristic

Retrieval and ranking are not globally optimal. If the shortlist is bad, the IP cannot recover the right queue later.

### 3. The catalog is local and bounded

This repo works over a local audio collection, not a large streaming-scale music universe.

### 4. Folder labels are coarse proxies

Folder names are convenient for analysis and mild diversity constraints, but they are not rich metadata.

### 5. Exact optimization is only final-stage

The exact solver only sees the final shortlist, not the entire retrieved pool.

### 6. No longitudinal feedback loop yet

There is no online preference update, skip feedback, or across-session representation accounting.

## Failed / Reverted Direction

A richer weighted feature design was tested with extra timbre, harmonic, and energy descriptors. It increased rebuild time dramatically without producing a clearly worthwhile gain in neighbor quality, so it was reverted.

That decision matters for how this repo should be read:

> the current version intentionally prefers a simpler, inspectable baseline over a larger but less clearly justified feature stack.
